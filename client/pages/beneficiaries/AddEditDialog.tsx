...
import { listFamilies, subscribeFamilies, upsertFamily, uid as familyUid, type Family, linkBeneficiary, unlinkBeneficiary } from "@/store/families";
...
function combineName(first: string, father: string, grandfather: string, family: string, tribe: string){
  return [first, father, grandfather, family, tribe].map(s=>s?.trim()).filter(Boolean).join(" ");
}
...
export default function AddEditBeneficiaryDialog({ open, onOpenChange, initial }: AddEditDialogProps) {
  ...
  useEffect(() => {
    if (open) {
      if (initial) {
        ...
        const linked = families.find(f => (f.links||[]).some(l => l.beneficiaryId === initial.id));
        setSelectedFamilyId(linked?.id || "");
      } else {
        setFirstName(""); setFatherName(""); setGrandfatherName(""); setFamilyName(""); setTribeName("");
        setGender("male");
        setDob("");
  ...
  const valid = useMemo(() => {
    ...
    const hasName = combineName(firstName, fatherName, grandfatherName, familyName, tribeName).trim().length > 0;
    const base = (
      (!r.name || hasName) &&
  ...
  }, [firstName, fatherName, grandfatherName, familyName, tribeName, dob, beneficiaryId, civilId, phone, guardianName, guardianPhone, settings, custom, gender]);
  ...
  function onSave() {
    ...
    const fullName = combineName(firstName, fatherName, grandfatherName, familyName, tribeName);
    const base = initial
      ? { ...initial }
    ...
    try {
      if (selectedFamilyId) {
        ...
        for (const f of all) {
          if (f.id !== selectedFamilyId && (f.links||[]).some(l => l.beneficiaryId === base.id)) {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-5xl max-h-[85vh] overflow-y-auto">
        ...
        <Tabs defaultValue="identity" className="mt-2">
          <TabsList className="flex flex-wrap">
            ...
            <TabsTrigger value="education">{ar?"التعليم":"Education"}</TabsTrigger>
            <TabsTrigger value="emergency">{ar?"الطوارئ":"Emergency"}</TabsTrigger>
            <TabsTrigger value="attachments">{ar?"المرفقات":"Attachments"}</TabsTrigger>
            {settings.customFields?.length ? (
              <TabsTrigger value="custom">{ar?"حقول إضافية":"Custom Fields"}</TabsTrigger>
            ) : null}
          </TabsList>
          ...
          <TabsContent value="identity" className="space-y-4 mt-4">
            ...
            <div className="grid md:grid-cols-2 gap-3">
              <div className="space-y-2">
                ...
                <select className="w-full h-9 rounded-md border bg-background px-3 text-sm" value={gender} onChange={(e)=>setGender(e.target.value as any)}>
                  {(settings.lists.gender || ["male","female"]).map((g:string)=> (
                    <option key={g} value={g as any}>{g}</option>
                  ))}
                </select>
              </div>
              <div className="space-y-2">
            ...
            <div>
              <Label>{ar?"الاسم وفق الصيغة السعودية":"Name (Saudi format)"} *</Label>
              <div className="grid md:grid-cols-3 gap-3 mt-2">
                <Input placeholder={ar?"الاسم الأول":"First Name"} value={firstName} onChange={(e)=>setFirstName(e.target.value)} />
                <Input placeholder={ar?"اسم الأب":"Father’s Name"} value={fatherName} onChange={(e)=>setFatherName(e.target.value)} />
                <Input placeholder={ar?"اسم الجد":"Grandfather’s Name"} value={grandfatherName} onChange={(e)=>setGrandfatherName(e.target.value)} />
                <Input placeholder={ar?"اسم العائلة":"Family Name"} value={familyName} onChange={(e)=>setFamilyName(e.target.value)} />
                <Input placeholder={ar?"القبيلة/اسم إضافي (اختياري)":"Tribe/Additional (optional)"} value={tribeName} onChange={(e)=>setTribeName(e.target.value)} />
              </div>
              <p className="text-xs text-muted-foreground mt-1">{combineName(firstName, fatherName, grandfatherName, familyName, tribeName)}</p>
            </div>

            <div className="space-y-2">
              <Label>{ar?"ربط بملف عائلة":"Link to Family Profile"}</Label>
              <div className="flex items-center gap-2 flex-wrap">
                <Popover open={familyPickerOpen} onOpenChange={setFamilyPickerOpen}>
                  <PopoverTrigger asChild>
                    <Button variant="outline" role="combobox" className="w-[320px] justify-between">
                      {selectedFamily ? (
                        <span className="truncate">{selectedFamily.familyId} • {selectedFamily.name || "—"} • {selectedFamily.contact.phone || ""}</span>
                      ) : (
                        ar?"اختر العائلة":"Select family"
                      )}
                        ...
                        <CommandGroup>
                          {families.map((f)=> (
                            <CommandItem key={f.id} value={`${f.familyId} ${f.name||""} ${f.contact.phone||""}`} onSelect={() => { setSelectedFamilyId(f.id); setFamilyPickerOpen(false); }}>
                              <Check className={cn("mr-2 h-4 w-4", selectedFamilyId===f.id?"opacity-100":"opacity-0")} />
                              <span className="truncate">{f.familyId} • {f.name || "—"} • {f.contact.phone || ""}</span>
                            </CommandItem>
                          ))}
                        </CommandGroup>
                      </CommandList>
                ...
                <Button variant="secondary" onClick={()=>setCreateFamilyOpen(true)}><Plus className="ml-1 h-4 w-4" /> {ar?"إنشاء عائلة جديدة":"Create new family"}</Button>
              </div>
            </div>
          </TabsContent>
          ...
          <TabsContent value="care" className="space-y-3 mt-4">
            <div className="grid md:grid-cols-2 gap-3">
              <div className="space-y-2">
                ...
                <select className="w-full h-9 rounded-md border bg-background px-3 text-sm" value={disability} onChange={(e)=>setDisability(e.target.value as any)}>
                  <option value="physical">{ar ? "حركية" : "Physical"}</option>
                  <option value="intellectual">{ar ? "ذهنية" : "Intellectual"}</option>
          ...
          <TabsContent value="attachments" className="space-y-3 mt-4">
            <div className="space-y-2">
              ...
              {photoDataUrl ? (
                <img src={photoDataUrl} alt="photo" className="h-20 w-20 object-cover rounded-md" />
              ) : null}
            </div>
            <div className="space-y-2">
              <Label>
      ...
      {createFamilyOpen && (
        <QuickCreateFamilyDialog
          ...
          onCreated={(fam)=>{ setSelectedFamilyId(fam.id); }}
        />
      )}
    </Dialog>
  );
...
function QuickCreateFamilyDialog({ open, onOpenChange, onCreated }:{ open: boolean; onOpenChange: (v:boolean)=>void; onCreated: (f: Family)=>void; }){
  const ar = getLocale()==="ar";
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>{ar?"إنشاء عائلة جديدة":"Create new family"}</DialogTitle>
          <DialogDescription>{ar?"أدخل معلومات أساسية لملف العائلة":"Enter basic info for the family profile"}</DialogDescription>
        </DialogHeader>
        <div className="grid md:grid-cols-2 gap-3">
          <div className="space-y-2"><Label>Family ID</Label><Input value={familyId} onChange={(e)=>setFamilyId(e.target.value)} placeholder="F-0001" /></div>
          <div className="space-y-2"><Label>{ar?"اسم العائلة (اختياري)":"Family name (optional)"}</Label><Input value={name} onChange={(e)=>setName(e.target.value)} /></div>
        ...
        <DialogFooter>
          ...
          <Button disabled={!valid} onClick={()=>{ const fam: Family = { id: familyUid(), familyId: familyId.trim(), name: name.trim()||undefined, contact: { phone: phone.trim()||undefined, email: email.trim()||undefined }, socio: {}, notes: undefined, guardians: [], links: [], documents: [] }; upsertFamily(fam); toast.success(ar?"تم إنشاء العائلة":"Family created"); onCreated(fam); onOpenChange(false); }}>{ar?"إنشاء":"Create"}</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
