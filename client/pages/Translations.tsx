...
export default function Translations() {
  ...
  const [log, setLog] = useState<
    Array<{ at: string; action: string; details: string }>
  >(() => {
    try {
      return JSON.parse(localStorage.getItem("i18n_logs_v1") || "[]");
    } catch {
      return [];
    }
  });
  ...
  async function scanProjectKeys() {
    ...
    pushLog({
      action: "scan",
      details: `${newlyAdded.length} keys added from ${found.size} files`,
    });
    toast.success(
      ar ? `تمت إضافة ${newlyAdded.length}` : `Added ${newlyAdded.length}`,
    );
  ...
  async function exportAllAsExcel() {
    ...
    const rows = allKeys.map((k) => [
      ...
      ...locales.map(
        (loc) =>
          getBaseMessage(loc as any, k) || getOverridesForLocale(loc)[k] || "",
      ),
    ]);
  ...
  async function importFromExcel(
    ...
    if (!json.length) {
      toast.error(ar ? "ملف فارغ" : "Empty file");
      return;
    }
    const header = (json[0] as any[]).map((h: any) => String(h || "").trim());
    const keyIdx = header.findIndex((h) => /^key$/i.test(h));
    if (keyIdx === -1) {
      toast.error(ar ? "عمود Key مفقود" : "Missing Key column");
      return;
    }
    const locales = header
      .filter((h, i) => i !== keyIdx && h)
      .map((h) => String(h));
    let added = 0,
      updated = 0,
      skipped = 0;
    ...
    <div className="space-y-6">
          ...
          <label className="inline-flex items-center gap-2">
            <input
              ...
              onChange={(e) => {
                ...
                if (!f) return;
                setConfirmImport({ mode: "incremental" });
                (e.target as any).value = "";
                (window as any).__pendingExcel = f;
              }}
            />
            <Button>{ar ? "استيراد Excel" : "Import Excel"}</Button>
          </label>
          <Button variant="outline" onClick={exportAllAsExcel}>
                ...
                <DropdownMenuItem
                  ...
                  onClick={() => {
                    const cols: ColumnDef<{
                      ...
                      {
                        header: ar ? "Arabic" : "Arabic",
                        accessor: (r) => r.currentAr,
                      },
                    ];
      ...
      <Card className="mt-6">
        <CardHeader>
          ...
          <CardDescription>
            {ar ? "آخر العمليات على الترجمات" : "Recent translation operations"}
          </CardDescription>
        </CardHeader>
        <CardContent>
      ...
      <AlertDialog open={confirmScan}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            <AlertDialogCancel onClick={() => setConfirmScan(false)}>
              {ar ? "إلغاء" : "Cancel"}
            </AlertDialogCancel>
            <AlertDialogAction
              onClick={async () => {
                setConfirmScan(false);
                await scanProjectKeys();
              }}
            >
              {ar ? "تأكيد" : "Confirm"}
            </AlertDialogAction>
      ...
      <AlertDialog open={!!confirmImport}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
              {ar ? "استيراد من Excel؟" : "Import from Excel?"}
            </AlertDialogTitle>
            <AlertDialogDescription>
              {ar ? "اختر وضع الاستيراد:" : "Choose import mode:"}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <div className="flex items-center gap-2 px-6">
            <Button
              variant={
                confirmImport?.mode === "incremental" ? "default" : "outline"
              }
              onClick={() => setConfirmImport({ mode: "incremental" })}
            >
              {ar ? "تحديث جزئي" : "Incremental"}
            </Button>
            <Button
              variant={
                confirmImport?.mode === "overwrite" ? "default" : "outline"
              }
              onClick={() => setConfirmImport({ mode: "overwrite" })}
            >
              {ar ? "استبدال كامل" : "Overwrite"}
            </Button>
          </div>
          <AlertDialogFooter>
            <AlertDialogCancel onClick={() => setConfirmImport(null)}>
              {ar ? "إلغاء" : "Cancel"}
            </AlertDialogCancel>
            <AlertDialogAction
              onClick={async () => {
                const f = (window as any).__pendingExcel as File | undefined;
                if (!f) {
                  setConfirmImport(null);
                  return;
                }
                const mode = confirmImport!.mode;
                setConfirmImport(null);
                await importFromExcel(f, mode);
                (window as any).__pendingExcel = undefined;
              }}
            >
              {ar ? "متابعة" : "Proceed"}
            </AlertDialogAction>
