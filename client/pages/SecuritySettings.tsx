    ...
    <div className="space-y-6">
      ...
      <Tabs defaultValue="auth">
        ...
        <TabsContent value="auth" className="mt-6">
          <AuthCard canManage={canManage} />
        </TabsContent>
        <TabsContent value="roles" className="mt-6">
          <RolesCard canManage={canManage} />
        </TabsContent>
        ...
        <TabsContent value="data" className="mt-6">
          <DataProtectionCard canManage={canManage} />
        </TabsContent>
        <TabsContent value="compliance" className="mt-6">
          <ComplianceCard canManage={canManage} />
        </TabsContent>
      </Tabs>
    </div>
  );
...
function AuthCard({ canManage }: { canManage: boolean }) {
  ...
  const [minLength, setMinLength] = useState(
    String(a.passwordPolicy.minLength),
  );
    ...
    <Card variant="modern">
      ...
      <CardContent className="space-y-4">
        <div className="flex items-center gap-2">
          <Checkbox
            checked={twoFactor}
            onCheckedChange={(v) => setTwoFactor(v === true)}
          />{" "}
        ...
        <div className="grid md:grid-cols-3 gap-3">
          ...
          <div className="flex items-center gap-2 mt-6">
            <Checkbox
              checked={autoLogout}
              onCheckedChange={(v) => setAutoLogout(v === true)}
            />{" "}
            <Label>Auto-logout on timeout</Label>
          </div>
        </div>
        <div>
          ...
          <div className="grid md:grid-cols-5 gap-4 mt-2">
            ...
            <label className="flex items-center gap-2 text-sm">
              <Checkbox
                checked={upper}
                onCheckedChange={(v) => setUpper(v === true)}
              />{" "}
              Uppercase
            </label>
            <label className="flex items-center gap-2 text-sm">
              <Checkbox
                checked={lower}
                onCheckedChange={(v) => setLower(v === true)}
              />{" "}
              Lowercase
            </label>
            <label className="flex items-center gap-2 text-sm">
              <Checkbox
                checked={num}
                onCheckedChange={(v) => setNum(v === true)}
              />{" "}
              Number
            </label>
            <label className="flex items-center gap-2 text-sm">
        ...
        <div className="flex justify-end">
          <Button
            ...
            onClick={() => {
              updateAuth({
                twoFactor,
                sessionTimeoutMin: Number(timeoutMin),
                autoLogout,
              });
              setPasswordPolicy({
...
function RolesCard({ canManage }: { canManage: boolean }) {
  ...
  const [financialTo, setFinancialTo] = useState<string[]>(
    s.sensitive.financialVisibleTo,
  );
  const [medicalTo, setMedicalTo] = useState<string[]>(
    s.sensitive.medicalVisibleTo,
  );
  function toggle(target: string[], set: (v: string[]) => void, id: string) {
    ...
    <Card variant="modern">
      ...
      <CardContent className="space-y-4">
        ...
        <Table>
          ...
          <TableBody>
            {roles.map((r) => {
              const cur = s.rolePerms.find((p) => p.roleId === r.id) || {
                roleId: r.id,
                view: true,
                edit: false,
                del: false,
              ...
              return (
                <TableRow key={r.id}>
                  ...
                  {(["view", "edit", "del", "export"] as const).map((k) => (
                    <TableCell key={k}>
                      <Checkbox
                        checked={(cur as any)[k]}
                        onCheckedChange={(v) => {
                          const next = { ...cur, [k]: v === true };
                          upsertRolePerm(next);
                        }}
                      />
                    </TableCell>
        ...
        <div>
              ...
              <div className="flex flex-wrap gap-2 mt-1">
                {roles.map((r) => (
                  <Button
                    ...
                    size="sm"
                    variant={
                      financialTo.includes(r.id) ? "default" : "secondary"
                    }
                    onClick={() => toggle(financialTo, setFinancialTo, r.id)}
                  >
                    {r.name}
                  </Button>
                ))}
              ...
              <div className="flex flex-wrap gap-2 mt-1">
                {roles.map((r) => (
                  <Button
                    key={r.id}
                    size="sm"
                    variant={medicalTo.includes(r.id) ? "default" : "secondary"}
                    onClick={() => toggle(medicalTo, setMedicalTo, r.id)}
                  >
                    {r.name}
                  </Button>
                ))}
          ...
          <div className="flex justify-end mt-3">
            <Button
              ...
              onClick={() => {
                setSensitiveAccess({
                  financialVisibleTo: financialTo,
                  medicalVisibleTo: medicalTo,
                });
                toast.success("Saved");
              }}
            >
          ...
          <TableBody>
            {s.audits.slice(0, 300).map((a) => (
              <TableRow key={a.id}>
                ...
                <TableCell>{a.userId || "system"}</TableCell>
...
function DataProtectionCard({ canManage }: { canManage: boolean }) {
  ...
  return (
    <Card variant="modern">
      ...
      <CardContent className="space-y-4">
        <div className="flex items-center gap-2">
          <Checkbox checked={enc} onCheckedChange={(v) => setEnc(v === true)} />{" "}
          <Label>Enable encryption for sensitive fields</Label>
        </div>
        <div className="grid md:grid-cols-3 gap-3">
          <div>
          ...
          <div>
            <Label>Archive records after (years)</Label>
            <Input
              type="number"
              value={years}
        ...
        <div className="flex justify-end">
          <Button
            ...
            onClick={() => {
              setDataProtection({
                encryptSensitive: enc,
                backupFrequency: freq,
                backupRetentionDays: Number(retDays),
                retentionYears: Number(years),
              });
              toast.success("Saved");
            }}
          >
    ...
    <Card variant="modern">
      ...
      <CardContent className="space-y-4">
        ...
        <div className="flex items-center gap-2">
          <Checkbox
            checked={hipaa}
            onCheckedChange={(v) => setHipaa(v === true)}
          />{" "}
        ...
        <div className="flex items-center gap-2">
          ...
          />{" "}
          <Label>Require consent (guardian/patient)</Label>
        </div>
        <div className="flex gap-2">
          <Button
            disabled={!canManage}
            onClick={() => {
              setCompliance({ gdpr, hipaa, consentRequired: consent });
              toast.success("Saved");
            }}
          >
            Save
