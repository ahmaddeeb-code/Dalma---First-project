...
export default function ChangePasswordDialog({ open, onOpenChange }: { open: boolean; onOpenChange: (v: boolean) => void }) {
  ...
  async function submit() {
    ...
    if (!strong.test(pw1)) { setErr("Password must be 8+ chars, include upper, lower, and a number"); return; }
    ...
    try {
      const r = await fetch("/api/auth/change-password", {
        ...
        body: JSON.stringify({ userId, currentPassword: current, newPassword: pw1 }),
      });
      const d = await r.json();
      if (!d.ok) {
        if (d.error === "Incorrect current password") setErr("Incorrect current password");
        else if (d.error === "Weak password") setErr("Password must be 8+ chars, include upper, lower, and a number");
      ...
      <DialogContent>
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-1">
            ...
            <div className="text-xs text-muted-foreground">8+ chars, include upper, lower, and a number.</div>
          </div>
          <div className="space-y-1">
