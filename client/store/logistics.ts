...
export function subscribeLogistics(cb: () => void) {
  subs.add(cb);
...
function seed(): LogisticsState {
  const b1 = {
    id: uid("b"),
    name: L("Main Building", "المبنى الرئيسي"),
    address: L("Street 1", "شارع ١"),
    floors: 3,
    capacity: 200,
    description: L("Primary facility", "المرفق الأساسي"),
    photo: "",
  };
  const r1 = {
    id: uid("r"),
    buildingId: b1.id,
    name: L("Therapy Room 101", "غرفة علاج ١٠١"),
    floor: 1,
    type: "therapy" as RoomType,
    capacity: 4,
    accessibility: [L("Wheelchair", "كرسي متحرك")],
    active: true,
  } as Room;
  const r2 = {
    id: uid("r"),
    buildingId: b1.id,
    name: L("Medical Office 201", "مكتب طبي ٢٠١"),
    floor: 2,
    type: "medical" as RoomType,
    capacity: 2,
    accessibility: [],
    active: true,
  } as Room;
  const s1: RoomSchedule = {
    id: uid("s"),
    roomId: r1.id,
    title: L("Physio Session", "جلسة علاج طبيعي"),
    kind: "therapy",
...
function load(): LogisticsState {
  ...
  try {
    cache = JSON.parse(raw) as LogisticsState;
    return cache!;
  } catch {
    cache = seed();
    localStorage.setItem(KEY, JSON.stringify(cache));
...
function save(state: LogisticsState) {
  cache = state;
  localStorage.setItem(KEY, JSON.stringify(state));
  notify();
}

export function getLogistics(): LogisticsState {
  return load();
}

function upsert<T extends WithId>(arr: T[], item: T): T[] {
...
function removeById<T extends WithId>(arr: T[], id: string): T[] {
  return arr.filter((x) => x.id !== id);
}

export function upsertBuilding(b: Building) {
  const s = load();
  save({ ...s, buildings: upsert(s.buildings, b) });
}
export function removeBuilding(id: string) {
  ...
  save({
    ...s,
    buildings: removeById(s.buildings, id),
    rooms: s.rooms.filter((r) => r.buildingId !== id),
    schedules: s.schedules.filter((sc) => !rooms.includes(sc.roomId)),
    equipment: s.equipment.filter((eq) => !rooms.includes(eq.roomId)),
  });
}

export function upsertRoom(r: Room) {
  const s = load();
  save({ ...s, rooms: upsert(s.rooms, r) });
}
export function removeRoom(id: string) {
  const s = load();
  save({
    ...s,
    rooms: removeById(s.rooms, id),
    schedules: s.schedules.filter((sc) => sc.roomId !== id),
    equipment: s.equipment.filter((eq) => eq.roomId !== id),
  });
}
...
export function hasScheduleConflict(
  ...
  for (const s of list) {
    ...
    if (candidate.recurrence.type === "none" && s.recurrence.type === "none") {
      ...
      const cDays =
        candidate.recurrence.type === "weekly"
          ? candidate.recurrence.days || []
          : [dayOfWeek(cStart)];
      const sDays =
...
export function upsertSchedule(sc: RoomSchedule): {
  ok: boolean;
  reason?: string;
} {
  const s = load();
  if (hasScheduleConflict(sc, s.schedules))
...
export function removeSchedule(id: string) {
  const s = load();
  save({ ...s, schedules: removeById(s.schedules, id) });
}

export function upsertEquipment(eq: Equipment) {
  const s = load();
  save({ ...s, equipment: upsert(s.equipment, eq) });
}
export function removeEquipment(id: string) {
  const s = load();
  save({ ...s, equipment: removeById(s.equipment, id) });
}
