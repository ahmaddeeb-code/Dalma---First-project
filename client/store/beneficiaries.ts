...
function seed(): Beneficiary[] {
  ...
  const b1: Beneficiary = {
    ...
    medical: {
      ...
      medications: [
        { name: "Melatonin", dosage: "3mg", schedule: "Before sleep" },
      ],
      allergies: ["Peanuts"],
    },
    care: {
      goals: [
        "Improve communication by 20%",
        "Increase attention span to 15 minutes",
      ],
    ...
    documents: [
      {
        ...
        title: "Disability Certificate 2024",
      ...
      messages: [
        {
          id: uid(),
          from: "staff",
          content: "Reminder: ABA session tomorrow at 10 AM.",
          date: iso(today),
        },
        {
          id: uid(),
    ...
    emergency: {
      contacts: [
        { name: "Khalid Al Saud", relation: "Father", phone: "+966500000010" },
      ],
  ...
  const b2: Beneficiary = {
    ...
    care: {
      goals: [
        "Improve mobility with wheelchair transfers",
      ...
      appointments: [
        {
          ...
          type: "Physiotherapy",
          date: daysFromNow(2),
          therapist: "Latifa Al Dossary",
        },
        {
          id: uid(),
          type: "Occupational",
          date: daysFromNow(10),
          therapist: "Noura Al Yamani",
        },
      ],
      nextReview: daysFromNow(14),
    },
    education: {
      programs: ["Workshops", "Recreation"],
      activities: ["Swimming"],
    },
    documents: [
      {
    ...
    financial: {
      ...
      paymentHistory: [
        { id: uid(), date: daysAgo(5), amount: 1500, method: "Card" },
      ],
    },
    communication: {
      messages: [
        {
          id: uid(),
          from: "system",
          content: "Therapy schedule updated.",
          date: iso(today),
        },
      ],
      notifications: [
        { id: uid(), text: "Upcoming review in 2 weeks.", level: "low" },
      ],
    },
    emergency: {
      contacts: [
        { name: "Huda Al Naim", relation: "Mother", phone: "+966500000012" },
      ],
  ...
  const b3: Beneficiary = {
    ...
    documents: [
      {
        id: uid(),
        type: "Disability Certificate",
        title: "Certificate 2023",
        issuedAt: daysAgo(320),
    ...
    communication: {
      ...
      notifications: [
        {
          id: uid(),
          text: "Document nearing expiry in 40 days.",
          level: "medium",
        },
      ],
    },
    emergency: {
      contacts: [
        { name: "Ali Mohammed", relation: "Father", phone: "+966500000013" },
      ],
...
export function newBeneficiary(partial: Partial<Beneficiary>): Beneficiary {
  const b: Beneficiary = {
    ...
    beneficiaryId:
      partial.beneficiaryId || `B-${Math.floor(Math.random() * 9000 + 1000)}`,
...
export function queryBeneficiaries(q: BeneficiaryQuery): Beneficiary[] {
  ...
  return data.filter((b) => {
    ...
    if (
      q.disability &&
      q.disability !== "all" &&
      b.medical.disabilityType !== q.disability
    )
      return false;
    ...
    if (
      q.program &&
      q.program !== "all" &&
      !b.education.programs.includes(q.program)
    )
      return false;
    if (
      q.therapist &&
      q.therapist !== "all" &&
      b.care.assignedTherapist !== q.therapist
    )
      return false;
    const age = getAge(b);
  ...
  for (const b of load()) {
    for (const ap of b.care.appointments) {
      ...
      if (d < now && ap.attended === false) {
        missed.push({
          id: b.id,
          text: `${b.name}: Missed ${ap.type} session`,
          level: "medium",
        });
      }
    }
    for (const doc of b.documents) {
      ...
      if (d <= soon) {
        ...
        expiring.push({
          id: b.id,
          text: `${b.name}: ${doc.title} expires in ${diff} days`,
          level: lvl,
        });
      }
    }
    if (b.care.nextReview) {
      ...
      if (diff <= 14 && diff >= 0) {
        reviews.push({
          id: b.id,
          text: `${b.name}: Review in ${diff} days`,
          level: diff <= 7 ? "medium" : "low",
        });
      }
    }
  }
